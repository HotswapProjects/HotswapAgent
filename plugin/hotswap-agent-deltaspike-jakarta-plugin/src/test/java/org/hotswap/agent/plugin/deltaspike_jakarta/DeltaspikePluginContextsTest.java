...
import org.hotswap.agent.plugin.deltaspike_jakarta.testBeans.WindowBean1;
import org.hotswap.agent.plugin.deltaspike_jakarta.testBeansHotswap.WindowBean2;
...
/**
 * Test Deltaspike Jakarta Plugin contexts, using OWB
 *
...
public class DeltaspikePluginContextsTest extends HAAbstractUnitTest {
    ...
    private void swapClasses(Class original, String swap) throws Exception {
        ...
        if (System.getProperty("cdicontainer.version").startsWith("weld")) {
            clazz = getClass().getClassLoader().loadClass("org.hotswap.agent.plugin.weld_jakarta.command.BeanClassRefreshAgent");
        } else {
            clazz = getClass().getClassLoader().loadClass("org.hotswap.agent.plugin.owb_jakarta.command.BeanClassRefreshCommand");
        }
