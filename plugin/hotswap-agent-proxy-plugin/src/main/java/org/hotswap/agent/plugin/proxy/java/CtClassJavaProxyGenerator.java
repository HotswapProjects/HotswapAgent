...
public class CtClassJavaProxyGenerator {
	 ...
	 */
	public static byte[] generateProxyClass(final String name, CtClass[] interfaces) {
	 ...
	 */
	public static byte[] generateProxyClass(final String name, CtClass[] interfaces, int accessFlags) {
		CtClassJavaProxyGenerator gen = new CtClassJavaProxyGenerator(name, interfaces, accessFlags);
	...
	private static CtMethod hashCodeMethod;
	private static CtMethod equalsMethod;
	private static CtMethod toStringMethod;
	private static ClassPool clp = ProxyTransformationUtils.getClassPool();
	private static CtClass oclp;
	private static CtClass error;
	private static CtClass runtimee;
	private static CtClass throwable;
	static {
		try {
			oclp = clp.get(Object.class.getName());
			error = clp.get(Error.class.getName());
			runtimee = clp.get(RuntimeException.class.getName());
			throwable = clp.get(Throwable.class.getName());
			hashCodeMethod = oclp.getDeclaredMethod("hashCode");
			equalsMethod = oclp.getDeclaredMethod("equals");
			toStringMethod = oclp.getDeclaredMethod("toString");
	 ...
	 */
	private CtClassJavaProxyGenerator(String className, CtClass[] interfaces, int accessFlags) {
	...
	private byte[] generateClassFile() {
		 ...
		 */
		addProxyMethod(hashCodeMethod, oclp);
		addProxyMethod(equalsMethod, oclp);
		addProxyMethod(toStringMethod, oclp);
	 ...
	 */
	private static List<CtClass> computeUniqueCatchList(CtClass[] exceptions) {
		List<CtClass> uniqueList = new ArrayList<>();
		// unique exceptions to catch
		
		uniqueList.add(error); // always catch/rethrow these
		uniqueList.add(runtimee);
		
		nextException: for (CtClass ex : exceptions) {
			if (isAssignableFrom(ex, throwable)) {
				/*
				...
				uniqueList.clear();
				break;
			} else if (!isAssignableFrom(throwable, ex)) {
				/*
