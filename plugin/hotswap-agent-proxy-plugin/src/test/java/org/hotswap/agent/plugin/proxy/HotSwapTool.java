...
public class HotSwapTool {
	...
	/**
	 * Prefix for the version number in the class name. The class bytes are modified that this string including the
	 * following number is removed. This means that e.g. A___2 is treated as A anywhere in the source code. This is
	 * introduced to make the IDE not complain about multiple defined classes.
	 */
	...
	private static Map<Class<?>, byte[]> buildRedefinitionMap(Map<String, File> classes) throws IOException,
		...
		for (String name : classes.keySet()) {
			...
			Class<?> replacement = (policy != null && policy.alias() != ClassRedefinitionPolicy.NoClass.class) ? policy
					.alias() : clazz;
	...
	private static void loadAdaptedClass(File file, Map<String, String> typeMappnigs, Map<Class<?>, byte[]> result)
